...
function icon(string $nome, string $w = '20', string $h = '20', string $extraClass = ''): string
{
    ...
    $icons = [
        ...
        'download' => '<svg xmlns="http://www.w3.org/2000/svg" width="'.$w.'" height="'.$h.'" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="'.$class.'"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" x2="12" y1="15" y2="3"/></svg>',

        'upload' => '<svg xmlns="http://www.w3.org/2000/svg" width="'.$w.'" height="'.$h.'" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="'.$class.'"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" x2="12" y1="3" y2="15"/></svg>',
    ];
...
function paginacao_html(int $total, int $porPagina, int $pagina, string $base, array $query = []): string
{
    ...
    $montar = static function (int $n) use ($paramsQuery): string {
        return ($paramsQuery === '' ? 'pagina=' . $n : $paramsQuery . '&pagina=' . $n);
    };
